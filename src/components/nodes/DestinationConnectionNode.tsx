...
> = ({ data }) => {
  ...
  const base64ToBlobImage = (base: string, fileType: string) => {
    try {
      // Check if the base64 string contains a base64 prefix and remove it
      const cleanedBase64 = base.includes("base64,")
        ? base.split("base64,")[1]
        : base;
  ...
  useEffect(() => {
    ...
    if (data.file?.content && data.file.filetype) {
      let blob;
      if (data.file.filetype === "image/png") {
        setFileContentUrl(`data:image/png;base64,${data.file.content}`);
      } else {
        blob = base64ToBlob(data.file.content, data.file.filetype);
        if (blob) {
    ...
    if (fileContentUrl) {
      ...
      a.download =
        data.file.filetype === "application/json"
          ...
          : data.file.filetype === "image/png"
          ? `${data.file.filename.split(".")[0]}.png`
